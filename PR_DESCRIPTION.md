...
2.  **Check Career Page:** Verify that jobs load dynamically with a spinner and display the current roles from the Dover API.
3.  **Check Homepage:** Verify the new H1, Service Cards, and the new **MCP Integration** section.
4.  **Check Sitemap:** Visit `http://localhost:3000/sitemap.xml` (after build) to ensure it lists all pages.
